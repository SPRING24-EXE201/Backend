...
class CabinetByLocation(serializers.ModelSerializer):
    location_detail = serializers.CharField(source='controller.location.location_detail', read_only=True)
    ward_name = serializers.CharField(source='controller.location.ward.name', read_only=True)
    description = serializers.CharField(read_only=True)
    id = serializers.IntegerField(read_only=True)
    district_name = serializers.CharField(source='controller.location.ward.district.name')
    province_name = serializers.CharField(source='controller.location.ward.district.province.name')
    empty_cells = serializers.SerializerMethodField(method_name='get_empty_cells')
    class Meta:
        model = Cabinet
        fields = ['location_detail', 'ward_name', 'description', 'id', 'district_name', 'province_name', 'empty_cells']
